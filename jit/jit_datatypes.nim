...
type
  ...
  Assembler*[R: enum] = object
    ...
    code_restore_regs*: ByteCode     # Return opcodes must restore clobbered registers
      # Even though code_restore_regs is known at compile-time
      # We still use a seq to avoid inflating the binary size
      # with one instantiation of each opcode per clobbered register
...
func post_process*(a: var Assembler) =
  ...
  for labelInfo in a.labels.values:
    for useAt in labelInfo.useAt:
      a.code[useAt ..< useAt + 4] = cast[array[4, byte]](uint32 labelInfo.pos)
